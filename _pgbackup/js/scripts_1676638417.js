...
document.querySelectorAll('.instruments__tabs-triggers__item').forEach((item) =>
	item.addEventListener('click', function (e) {
		...
		document.querySelectorAll('.instruments__tabs-content__item').forEach(
			(child) => child.classList.remove('instruments__tabs-content--active')
		);

		item.classList.add('instruments__tabs-triggers__item--active');
		document.getElementById(id).classList.add('instruments__tabs-content-item--active')
	})
);
